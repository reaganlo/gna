...
void AffineKernelImpl2B(AffineConfig const * const config)
{
    ...
    if (1 == config->inputVectorCount)
    {
        ...
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            *output = vec_sum(acc0) + getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias);
    ...
    if (2 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[0] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc0);
            output[1] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
    ...
    if (3 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[0] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc0);
            output[1] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
            output[2] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
    ...
    if (4 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[1] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
            output[2] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
            output[3] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
    ...
    if (5 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[2] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
            output[3] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
            output[4] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
    ...
    if (6 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[3] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
            output[4] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
            output[5] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
    ...
    if (7 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[4] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
            output[5] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
            output[6] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc6);
    ...
    if (8 == config->inputVectorCount)
    {
        for (; bias < biasEnd; bias += config->bytesPerBias)
        {
            ...
            output[5] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
            output[6] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc6);
            output[7] = getBias((void*)bias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc7);
...
void AffineMultiBiasKernelImpl2B(AffineConfig const * const config)
{
    ...
    if (1 == config->inputVectorCount)
    {
        ...
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            *output = vec_sum(acc0) + getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias);
            for (i = 0; i < KT; i++, weight++)
            {
                *output += input_0[i] * *weight;
            }
    ...
    if (2 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[0] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc0);
            output[1] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
    ...
    if (3 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[0] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc0);
            output[1] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
            output[2] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
    ...
    if (4 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[1] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc1);
            output[2] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
            output[3] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
    ...
    if (5 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[2] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc2);
            output[3] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
            output[4] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
    ...
    if (6 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[3] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc3);
            output[4] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
            output[5] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
    ...
    if (7 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[4] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc4);
            output[5] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
            output[6] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc6);
    ...
    if (8 == config->inputVectorCount)
    {
        for (multiBias = (uint8_t*)config->multiBias; multiBias < biasEnd; multiBias+=(config->multiBiasVectorCount*config->bytesPerBias))
        {
            ...
            output[5] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc5);
            output[6] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc6);
            output[7] = getBias((void*)multiBias, 0, (gna_data_mode)config->bytesPerBias) + vec_sum(acc7);
